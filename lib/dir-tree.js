...
class DirTreeNode {
  constructor({ path, title, extension, source, type, model, absolutePath = null, children = [], active = true }) {
    this.path = path;
    this.title = title;
...
export function loadDirAsTree({input, models, subpath = [], ignoredFiles = [], includedFiles = []}) {
  ...
  const addNode = (path, title, extension, source, type, model) => {
    ...
    const newNode = new DirTreeNode({ 
      ...
      active: getNodeActiveState(type, path, ignoredFiles, includedFiles),
    });
    nodes.push(newNode);
    return newNode;
  };
  ...
  for (const file of files) {
    ...
    if (!isIncludedFile(filePath)) continue;
