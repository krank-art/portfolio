...
class DirTreeNode {
  constructor({ path, title, extension, source, type, model, absolutePath = null, children = [], rendered = true }) {
    this.path = path;
    this.title = title;
    ...
    //this.parent = parent;
    // Inactive node will not get built, but are visible in the dirtree. This is intended so 
    // you can skip building expensive pages, but still have the full page hierarchy during development.
 ...
 * @returns {DirTreeNode[]}
 */
export function loadDirAsTree({input, models, subpath = [], ignoredFiles = [], includedFiles = []}) {
  // Setup
  const nodes = [];
  const files = fs.readdirSync(input);
  const getNodeActiveState = (nodeType, filePath, ignoredFiles, includedFiles) => {
    if (nodeType === "group") return true;
    if (ignoredFiles.includes(filePath)) return false;
    if (includedFiles.length <= 0) return true;
    return includedFiles.includes(filePath);
  };
  ...
  const addNode = (path, title, extension, source, type, model) => {
    ...
    if (existingNode && existingNode.type === "group") {
      ...
      existingNode.rendered = getNodeActiveState(type, source, ignoredFiles, includedFiles);
      return existingNode;
    }
    if (existingNode)
      throw new Error(`Cannot override existing page type '${existingNode.type}' of '${existingNode.path}'. `);
    const newNode = new DirTreeNode({ 
      path, title, extension, source, type, model, children: [],
      rendered: getNodeActiveState(type, source, ignoredFiles, includedFiles),
    });
    nodes.push(newNode);
    return newNode;
  };
