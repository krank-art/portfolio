import { concatArrayBuffers } from "./buffer.js";
import { invertObject } from "./object.js";

// INFO: Make sure to mirror all changes here in /www/php/comment-decoder.php
// This is the client side decoding and encoding. When we send and receive the comment history,
// we want to use the binary format for exchange to save on network resources (binary format takes up about
// 6% of the raw JSON data).
...
function encodeSignedLEB8(value) {
  /*
...
function decodeSignedLEB8(nibbles) {
  let shift = 0, result = 0, value;
  ...
  // WARNING: Numbers higher than 2^30 are unhandled (10 nibble groups).
  //  Thats 1.073.741.824 in decimal, which for pratical reasons will not be reached.
...
function encodeStrokes(strokes) {
  ...
  for (const stroke of strokes) {
    ...
    for (const pt of path) {
      ...
      prevX = pt.x;
      prevY = pt.y;
      nibbles.push(...encodeSignedLEB8(dx));
      nibbles.push(...encodeSignedLEB8(dy));
    }
...
export function decodeStrokes(buffer, offset = 0) {
  const view = new DataView(buffer);
  const strokes = [];
  while (offset < buffer.byteLength) {
    ...
    while (nibbleLEBs.length < nibbleCount && offset < buffer.byteLength) {
      ...
      const nibbles = [(byte & 0xF0) >>> 4, byte & 0x0F];
      for (const nibble of nibbles) {
        if (nibbleLEBs.length >= nibbleCount) break;
        const isLeadingGroup = (nibble & 0b1000) === 0b1000;
    ...
    // Decode deltas from LEB128 (4-bit version) aka LEB8
    const path = [];
    let prevX = 0, prevY = 0;
    for (let i = 0; i < nibbleLEBs.length; i += 2) {
      const firstNibbleSegment = nibbleLEBs[i];
      const secondNibbleSegment = nibbleLEBs[i + 1];
      const dx = firstNibbleSegment ? decodeSignedLEB8(firstNibbleSegment) : 0;
      const dy = secondNibbleSegment ? decodeSignedLEB8(secondNibbleSegment) : 0;
      const x = prevX + dx;
      const y = prevY + dy;
      path.push({ x, y });
      prevX = x;
    ...
    strokes.push({ brush, pattern, size, path });
  }
...
function decodeFileHeader(buffer) {
  const view = new DataView(buffer);
  ...
  return { version, width, height, strokeCount, crc, offset };
}

export function encodeCommentHistory({ version, width, height, strokes }) {
  ...
  return concatArrayBuffers([headerBuffer, buffer]);
}

export function decodeCommentHistory(buffer) {
  const { version, width, height, offset } = decodeFileHeader(buffer);
  const strokes = decodeStrokes(buffer, offset);
  return { version, width, height, strokes };
}
