import { setupCommentPlayer } from "./comment-manager";
...
export async function setupCommentList({ sidebar }) {
  ...
  try {
    ...
    comments.forEach(comment => {
      ...
      const asideArea = `
        <div class="comment-aside">
          <a class="button-secondary comment-referral" href="${comment.website}" target="_blank" rel="external"
              title="Website: ${comment.website}" >🏠 ${comment.website}</a>
        </div>
      `;
      ...
      item.innerHTML = `
        <div class="comment" id="${commentId}">
          ...
          <div class="comment-body">
            <div class="comment-replayable" data-history-src="${comment.historyPath}">
    ...
    await setupCommentPlayer({ width: 320, height: 120, scale: 1, target: listing});
  } catch (err) {
