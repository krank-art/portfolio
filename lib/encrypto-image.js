/*
 * This file is obsolete. It was a previous approach to encode images
 * in the least significant bytes of another image (steganography).
 * This is being surpassed by fully encrypting the original image
 * since it's a lot more space efficient.
 */

//import { PNG } from "pngjs"; // removed vendor lib
import { numberToUint32BitString } from './maths.js';


export function encryptBuffer(buffer, key, iv) {
...
function encryptImage({ inputFilePath, outputFilePath, key, iv }) {
  ...
  const encryptedImage = encryptBuffer(imageBuffer, key, iv);
...
export function createTestPNG() {
  ...
  for (let i = 0; i < width * height; i++) {
    ...
    png.data[idx] = 255 & dataMask | r;      // Store R with last two bits as `r`
    png.data[idx + 1] = 255 & dataMask | g;  // Store G with last two bits as `g`
    png.data[idx + 2] = 255 & dataMask | b;  // Store B with last two bits as `b`
    png.data[idx + 3] = 255 & dataMask | a;  // Store A with last two bits as `a`
  }
