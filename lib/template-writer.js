...
export default class TemplateWriter {
  ...
  async renderLayoutSfcNew(layoutName, dataChunk, defaultLayout = "default") {
    const layoutPath = path.join("layouts", layoutName + ".hbs");
    const inputSfc = TemplateLoader.readSfc(layoutPath);
    const preparedSfc = await TemplateEngine.prepareSfcNew({
      html: inputSfc.html,
  ...
  async compileSfcNew({ input, dataChunk }) {
    ...
    return await this.compileSfcNew({
      input: layoutRaw,
      dataChunk: preparedDataChunk,
    });
  }

  async compileMarkdownNew({ input, dataChunk, defaultLayout = "blog", middleware = [] }) {
    ...
    const renderedTemplate = await this.compileSfcNew({
      input: layoutRaw,
      dataChunk: preparedDataChunk,
    });
  ...
  */
  ...
  async compileEntry(source, extension, dataChunk) {
    ...
    if (extension === ".md") {
      return await this.compileMarkdownNew({
        input: source,
