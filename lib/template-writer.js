...
export default class TemplateWriter {
  ...
  async renderLayoutSfc(layoutName, data) {
    if (layoutName === "blog")
      console.log("blog sandwich")
  ...
  async compileMarkdown({ input, output, data, buildCache = null }) {
    ...
    const renderedTemplate = await this.renderLayoutSfc(layout, layoutData);
    const inputFileName = path.basename(input, path.extname(input));
  ...
  static loadDirAsTree(input, data, subpath = []) {
    ...
    const nodes = [];
    const files = fs.readdirSync(input);
    const findNode = (name) => nodes.find(node => node.name === name) ?? null;
    const addNode = (name, extension, type, model) => {
      const existingNode = findNode(name);
      ...
      if (existingNode)
        throw new Error(`Cannot override existing page type '${existingNode.type}' of '${existingNode.name}'. `);
      const newNode = { name, extension, type, model, children: [] };
      nodes.push(newNode);
      return newNode;
    };
    // Iterate over files
    for (const file of files) {
      ...
      const stats = fs.statSync(filePath);
      if (stats.isDirectory()) {
        const parentNode = findNode(file) ?? addNode(file, "", "group");
      ...
      if (extension === ".hbs") {
        ...
        if (!model) {
          addNode(basename, extension, "page");
          continue;
        }
        ...
        templateNodes.forEach(target => addNode(target, extension, "page", model));
        continue;
      }
      addNode(basename, extension, "page");
    }
