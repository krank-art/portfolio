...
import { hash8, interpolateString } from './string.js';
...
export default class TemplateWriter {
  ...
  async compileSfc({ input, dataChunk, previousLayout = null, deferredInserts = [] }) {
    ...
    if (!layout) {
      // Inject deferred HTML rendering
      let finalHtml = renderedHtml;
      for (const deferredInsert of deferredInserts) {
        const { hash, injectedHtml } = deferredInsert;
        const hashMarker = TemplateWriter.makeInjectionMarker(hash);
        finalHtml = finalHtml.replace(hashMarker, injectedHtml);
      }
      // Exit the recursion loop and return HTML
      return finalHtml;
    }

    // If we have a layout set, we need to render the parent layout once more (recursion)
  ...
  async compileMarkdown({ input, dataChunk, defaultLayout = "blog", middleware = [] }) {
    ...
    /* We do not run the templating engine on the raw markdown.
     * This means we cannot insert variables into markdown documents, but honestly it has not been necessary yet.
     * For now I rather want to be able to write curly braces (and other Handlebars syntax) without it being interpreted.
     */
    //const renderedHtml = this.engine.process(template, preparedDataChunk.flatten());
    const transformedHtml = middleware.length > 0
      ? middleware.reduce((result, currentFunction) => currentFunction(result), template)
      : template;
    const hash = hash8(title);
    // We need to first render all SFC templates and then inject the markdown result afterwards.
    // Otherwise curly braces get interpreted as Handlebars markup (and similar unwanted side effects).
    preparedDataChunk.addPayload("md-sfc", { ...yaml, content: TemplateWriter.makeInjectionMarker(hash) });
    return await this.compileSfc({
      input: layoutRaw,
      dataChunk: preparedDataChunk,
      deferredInserts: [{hash: hash, injectedHtml: transformedHtml}],
    });
  }
  ...
  */
  async readAndWriteQueue({ queue, output, cachePath = null, silent = false, profiler = null }) {
    ...
    for (const entry of queue) {
      ...
      const { source: inputPath, absolutePath: pagePath, type: pageType, rendered} = page;
  ...
  static makeInjectionMarker(id) {
    return `<div data-md-slot="${id}"></div>`;
  }
}
