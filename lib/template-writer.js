...
export default class TemplateWriter {
  ...
  static loadDir(input, data, subpath = []) {
    const targets = [];
    const files = fs.readdirSync(input);
    const addTarget = (path, file, extension, model) => targets.push({ path,file,extension, model});
    for (const file of files) {
      const filePath = path.join(input, file);
      const stats = fs.statSync(filePath);
      if (stats.isDirectory()) {
        const childTargets = TemplateWriter.loadDir(filePath, data, [...subpath, file]);
      ...
      if (extension === ".hbs") {
        ...
        if (!model) {
          addTarget([...subpath], basename, extension);
          continue;
        }
        ...
        const templateTargets = payload.map(item => item.path);
        templateTargets.forEach(target => addTarget([...subpath], target, extension, model));
        continue;
      }
      addTarget([...subpath], basename, extension);
    }
