...
export default class TemplateWriter {
  ...
  async renderLayoutSfc(layoutName, data, depth = 0) {
    ...
    if (!layout || layout === layoutName)
      return renderedHtml;
    return await this.renderLayoutSfc(layout, { ...templateData, content: renderedHtml }, depth);
  }
  ...
  async compileSfc(input, output, data, depth = 0, buildCache = null) {
    ...
    for (const pagePath in pageByPath) {
      ...
      const renderedTemplate = await this.renderLayoutSfc(payload.layout, layoutData, depth);
  ...
  async compileMarkdown({ input, output, data, depth = 0, buildCache = null }) {
    ...
    data.path.relative = "../".repeat(depth);
    ...
    const renderedTemplate = await this.renderLayoutSfc(layout, layoutData, depth);
    const inputFileName = path.basename(input, path.extname(input));
  ...
  static loadDirAsTree(input, data, subpath = []) {
    ...
    for (const file of files) {
      ...
      if (extension === ".md") {
        ...
        const { yaml } = TemplateLoader.parseMarkdown(markdownFileContent);
        const markdownLinkTitle = yaml?.linkTitle ?? basename;
