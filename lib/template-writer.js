...
export default class TemplateWriter {
  ...
  async compileSfc(input, output, data, depth = 0) {
  ...
  async compileSfcDir(input, output, data, subpath = []) {
    ...
    for (const file of files) {
      ...
      if (stats.isDirectory()) {
        await this.compileSfcDir(filePath, output, data, [...subpath, file])
        continue;
      }
      if (path.extname(filePath) !== ".hbs")
        continue;
      ensureDirExists(outputPath + path.sep);
      await this.compileSfc(filePath, outputPath, data, subpath.length);
    }
