...
export default class TemplateEngine {
  ...
  static processSfc(input, data, depth = 0) {
    const output = {};
    ...
    for (const path in output) {
      const pathData = isDynamicSfc
        ...
        : { ...extendedData };
      const renderedHtml = TemplateWriter.renderHtml(html, pathData, myHandlebars);
      const layoutData = { content: renderedHtml, style: style, ...pathData };
      const renderedTemplate = TemplateWriter.renderTemplate(`layouts/${layout}.hbs`, layoutData, myHandlebars);
