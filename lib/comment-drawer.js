class CanvasHistory {
  constructor({ maxSize = 20, callbacks }) {
    ...
    // We can initialize a canvas history with previous shadow states, this is intended to load inputs from
    // an external source and then allow the comment drawer to replay those strokes.
    // IMPORTANT: We cannot click "Undo" or "Redo" when preloading inputs, because no image data per state is saved.
  }
  ...
  add({ brush, size, pattern, path, pixelData }) {
    ...
    this.states.push({
      ...
      pattern,
      path,
      pixelData: pixelData ? CanvasHistory.duplicateImageData(pixelData) : null,
    });
    this.sequenceNumber++;
  ...
  preloadInputs(inputs) {
    for (const record of inputs) {
      const { brush, size, pattern, path } = record;
      this.add({ brush, size, pattern, path, pixelData: null });
    }
  }
}

const CommentMode = Object.freeze({
  Drawer: "drawer",
  Player: "player",
});

export default class CommentDrawer {
  constructor({ width, height, scale, container, onSubmit, mode = CommentMode.Drawer }) {
    // Static properties
    this.width = width;
    this.height = height;
    this.scale = scale;
    ...
    this.mode = mode === CommentMode.Player ? CommentMode.Player : CommentMode.Drawer;
    this.canvasHolder = document.createElement("div");
    this.canvasHolder.dataset.controlId = "comment-board";
    if (this.mode === CommentMode.Drawer) 
      this.initTemplate();
    this.canvas = CommentDrawer.initCanvas({
      ...
      hasDrawingTools: this.isModeDrawer,
    });
    ...
    this.history = new CanvasHistory({
      callbacks: this.isModeDrawer ? [(() => { this.updateButtonControls() }).bind(this)] : [],
    });

    // Dynamic properties
    this.acceptsInput = this.isModeDrawer;
    this.pattern = null;
    this.brush = null;
    this.size = -1;
    this.drawing = false;
    ...
    // Initialize controls
    if (this.isModeDrawer) {
      this.initMouseControls();
      this.initButtonControls();
      this.initHotkeys();
    }
    this.clear(this.isModeDrawer);
  }
  ...
  get isModeDrawer() {
    return this.mode === CommentMode.Drawer;
  }

  static initCanvas({ width, height, scale, canvasHolder, hasDrawingTools }) {
    const canvas = document.createElement("canvas");
    canvas.classList.add("comment-canvas");
    if (hasDrawingTools)
      canvas.classList.add("comment-drawer");
    canvas.width = width;
  ...
  drawOnCanvas(clientX, clientY) {
    ...
    if (x === this.lastKnownPosition.x && y === this.lastKnownPosition.y)
      return; // We do not want to record events that practically have no effect, it clogs up the history
  ...
  initTemplate() {
    /* Resulting structure in mode **PLAYER** is a lot simpler:
      *  <div class="comment-playable">
      *    <img src="my-comment.png">
      *    <div data-control-id="comment-board">
      *      <canvas ... >
      *    </div>
      *  </div>
      */
    const createRadio = ({ radioName, value, checked, icon, label }) => `
    <label class="row-multi-button" title="${label}">
  ...
  `;
    ...
    <button type="button" class="row-button ${classes ?? ''}" data-control-id="${buttonId}" title="${label}">
      <i class="row-button-icon ${icon}"></i>
      <span class="row-button-text">${label}</span>
    </button>
  `;
    ...
    `;
    const toolsMain = this.container.querySelector("[data-control-id='tools-main']");
    toolsMain.before(this.canvasHolder);
  }
  ...
  initButtonControls() {
    ...
    this.submitButton.addEventListener("click", () => {
      ...
      // Watch out, inputs are passed by reference!
      // We only send inputs since the last clear bc otherwise the history just gets too large.
      // This also aligns with the playback since only the inputs since last clear are shown.
      const imageActions = this.history.inputsSinceLastClear;
    ...
    this.aboutButton.addEventListener("click", () => {
      alert(
`Hotkeys are also available but work only in Fullscreen:

    1/2/3/4 \u2013 Brush strength 1/2/3/4
    5/6/7/8 \u2013 Pattern 100%/75%/50%/25%
    B \u2013 Brush
    E \u2013 Eraser
    R \u2013 Reset (Clear)
    P \u2013 Playback
...
Comment Drawer created by Krank (c) 2025`);
  ...
  attachToElement(targetElement) {
    if (this.mode !== CommentMode.Player) throw new Exception(`Must be in mode player`);
  ...
  show() {
    if (this.mode !== CommentMode.Player) throw new Exception(`Must be in mode player`);
    this.canvasHolder.style.display = "";
  }
  ...
  clear(addToHistory = true) {
    ...
    this.flush();
    if (addToHistory)
      // It would be ideal to save clear with 'size', 'pattern' and 'path' = null. But because of our binary 
      // serialization format, we cannot encode null. So we will use the next best logical values, that can actually
      // be represented with the current file specification.
      this.remember({
        brush: "clear",
        size: 0,
        pattern: "100%",
  ...
  resize({ scaleOverride = null, ignoreHeight = false }) {
    ...
    const controlsHeight = this.isModeDrawer ? 108 : 0;
    ...
    this.scale = Math.max(scaleOverride ?? sizeFactor, 1); // Cannot go lower than scale of 1
    this.container.dataset.scale = this.scale;
    // Due to performance reasons, it makes a lot more sense to keep the canvas in it's 1:1 pixel scale and instead
    // increase the canvas width + height and set the display to `image-rendering: pixelated;`.
    // The display size (CSS) is independent of the canvas pixel size (canvas size is tied to image data).
  ...
  loadHistory(inputs) {
    this.history.reset();
    this.history.preloadInputs(inputs);
  }
  ...
  playback(speed = 10, onEnd = null) {
    this.playbackTimers.forEach(id => clearTimeout(id));
    this.playbackTimers = [];
    if (this.isModeDrawer) {
    ...
    for (const action of this.history.inputsSinceLastClear) {
      ...
      if (brush === "clear") {
        ...
        totalDelay += bigDelay;
        this.playbackTimers.push(clearTimerID);
        continue;
      }
      ...
      for (let i = 0; i < maxIndex; i++) {
        ...
        totalDelay += delay;
        this.playbackTimers.push(drawTimerID);
      }
