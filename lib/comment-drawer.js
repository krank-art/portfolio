class CanvasHistory {
  constructor({ maxSize = 20, callbacks, preloadedInputs = null }) {
    ...
    this.preloadedInputs = preloadedInputs;
    this.preloadInputs();
  }
  ...
  preloadInputs() {
    if (!this.preloadedInputs) return;
    for (const record of this.preloadedInputs) {
      const { brush, size, pattern, path } = record;
...
export default class CommentDrawer {
  constructor({ width, height, scale, container, onSubmit, mode = CommentMode.Drawer, preloadedInputs = [] }) {
    // Static properties
    this.width = width;
    this.height = height;
    this.scale = scale;
    ...
    this.preloadedInputs = this.isModeDrawer ? null : preloadedInputs;
    this.initTemplate();
    ...
    this.history = new CanvasHistory({
      ...
      preloadedInputs: preloadedInputs,
    });
  ...
  static initCanvas({ width, height, scale, container, hasDrawingTools }) {
  ...
  initTemplate() {
    if (!this.isModeDrawer) {
      ...
      canvasHolder.dataset.controlId = "comment-board";
      this.container.appendChild(canvasHolder);
      return;
    }
