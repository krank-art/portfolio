...
export default class CommentDrawer {
  constructor({ width, height, scale, container, onSubmit }) {
    ...
    this.toolBarEditing = this.container.querySelector("[data-control-id=tools-editing]")
    this.toolBarMain = this.container.querySelector("[data-control-id=tools-main]")
    this.resize({});
  ...
  `;
    ...
    const fullscreenButton = createButton({ buttonId: "fullscreen", icon: "bi-save", label: "Fullscreen", classes: null }); // TODO: Fullscreen icon
    ...
    <div class="comment-drawer-inner">
      <div data-control-id="tools-editing" class="tool-bar">
        <div class="row-button-group">${brushRadios.map(radio => createRadio(radio)).join("")}</div>
        <div class="row-button-group">${eraserRadios.map(radio => createRadio(radio)).join("")}</div>
  ...
  initButtonControls() {
    ...
    this.submitButton.addEventListener("click", () => {
      ...
      const imageRaw = this.downloadImage();
    ...
    document.addEventListener('fullscreenchange', () => {
      ...
      this.resize({ ignoreHeight: !isFullscreen });
    });
  ...
  resize({ scaleOverride = null, ignoreHeight = false }) {
    ...
    this.toolBarEditing.dataset.scale = this.scale;
    this.toolBarMain.classList.scale = this.scale;
