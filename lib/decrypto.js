...
export function setupPostDecryption() {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const unlockableList = document.querySelector("[data-encryption-list]");
    if (!unlockableList) return;
    const unlockablePosts = Array.from(unlockableList.querySelectorAll("[data-encryption-item]"));
    if (unlockablePosts.length <= 0) return;
    const unlocker = new PostUnlocker(unlockingMask, unlockablePosts);
  });
}

class PostUnlocker {
  constructor(element, unlockablePostElements) {
    this.element = element;
    this.unlockablePosts = unlockablePostElements.map(rawPost => new UnlockablePost(rawPost));
...
class UnlockablePost {
  constructor(element) {
    this.element = element;
    this.hash = this.element.dataset.encryptionItem;
    const encryptionTagsList = Array.from(this.element.querySelectorAll("[data-enc-tag]"));
    const encryptionTags = Object.fromEntries(encryptionTagsList.map(el => [el.dataset.encTag, el]));
    this.title = encryptionTags.title;
    this.placeholder = encryptionTags.placeholder;
    this.imageThumb = encryptionTags["image-thumb"];
    this.imageSmall = encryptionTags["image-small"];
    this.date = encryptionTags.date;
    this.dimensions = encryptionTags.dimensions;
    this.fileSize = encryptionTags["file-size"];
  }

  async checkKey(aes128key) {
    //const fetch()
  }
}
