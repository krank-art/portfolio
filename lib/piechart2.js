...
// Made by Krank just cause, edit labels in Inkscape if colliding.
...
function generateExampleChart() {
  ...
  svg.append("g")
    ...
    .each(function (d) {
      ...
      d.arcCenter = {
        x: Math.cos(angleCoords) * (radius - 75),
        y: Math.sin(angleCoords) * (radius - 75),
      };
      d.labelOrigin = {
        x: Math.cos(angleCoords) * labelRadius,
        y: Math.sin(angleCoords) * labelRadius,
      };
      ...
      d.getBBox = () => {
        return {
          x: d.textAlign === "start" ? d.labelOrigin.x : d.labelOrigin.x - d.width,
          y: d.labelOrigin.y - d.height * yAnchorOffset,
          width: d.width,
          height: d.height,
        };
      };
      ...
      const bounds = d.getBBox();
      d.sx = d.textAlign === "start" ? d.labelOrigin.x : d.labelOrigin.x - bounds.width;
      d.ox = d.textAlign === "start" ? d.labelOrigin.x + bounds.width : d.labelOrigin.x;
      d.sy = d.labelOrigin.y;
      d.oy = d.labelOrigin.y;
    })
    .attr("x", d => d.labelOrigin.x)
    .attr("text-anchor", d => d.textAlign)
    .call(text => text.append("tspan")
      .attr("x", d => d.labelOrigin.x)
      .attr("y", d => d.getBBox().y + fontSize)
      .attr("font-weight", "bold")
      .text(d => d.labelTitle))
    .call(text => text.append("tspan")
      .attr("x", d => d.labelOrigin.x)
      .attr("y", d => d.getBBox().y + d.getBBox().height - 0)
  ...
  svg.append("g").selectAll("path.pointer").data(pieData).enter()
    ...
    .attr("d", function (d) {
      return d.arcCenter.x > d.ox
        ? `M${d.sx},${d.sy}L${d.ox},${d.oy} ${d.arcCenter.x},${d.arcCenter.y}`
        : `M${d.ox},${d.oy}L${d.sx},${d.sy} ${d.arcCenter.x},${d.arcCenter.y}`;
    });
  ...
  if (debug)
    // Enter selection: create new rects for each data point
    svg.append("g").selectAll("rect").data(pieData).enter()
      .append("rect")
      .attr("x", d => d.getBBox().x)
      .attr("y", d => d.getBBox().y)
      .attr("width", d => d.getBBox().width)
      .attr("height", d => d.getBBox().height)
      .attr("fill", "none")
      .attr("stroke", "red")
      .attr("stroke-width", 1);

  if (debug)
    svg.append("g").selectAll("circle").data(pieData).enter()
      .append("circle")
      .attr("cx", d => d.labelOrigin.x)
      .attr("cy", d => d.labelOrigin.y)
      .attr("r", 4)
      .attr("fill", "green");
