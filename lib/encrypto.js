...
import { formatDateYYYYMMDD } from './string';
...
function encryptSets(sets, outputDir) {
  // File basename must be unique
}

function generateIVs(count, fixedField = crypto.randomBytes(4)) {
  ...
  if (count > 2 ** 32) throw new Exception("Count exceeds maximum count of 2^32");
  ...
  const ivs = Array.from({ length: count }, () => Buffer.alloc(12));
  for (let i = 0; i < count; i++) {
    const counterBuffer = Buffer.alloc(8);
    counterBuffer.writeUInt32BE(count);
  }
  return ivs;
}

async function generateKeyAES128(password) {
...
/**
 * Encrypt a single file with AES-128-GCM.
 * @param {string} inputPath - Path to input file.
 * @param {string} outputPath - Path to output file.
 * @param {Buffer} key - 16-byte encryption key.
 * @param {Buffer} key - 12-byte initialization vector, also nonce. MUST BE UNIQUE PER KEY OR ELSE ENCRYPTION IS COMPROMISED.
 */
function encryptFile(inputPath, outputPath, key, iv) {
  ...
  // Wait for the streams to finish, then append IV + auth tag
  return new Promise((resolve, reject) => {
    output.on("finish", () => {
      const authTag = cipher.getAuthTag();
      fs.appendFileSync(outputPath, Buffer.concat([iv, authTag]));
      resolve();
    });
...
function encryptData({
  data, key, iv, salt, author = "https://krank.love/", encryptionDate = Date.now(), version = 1, magicBytes = "KENC"
}) {
  ...
  return Buffer.concat([fileHeader, ciphertext]);
}
...
function createFileHeader({ salt, iv, payloadLength, author, encryptionDate, version, magicBytes }) {
  ...
  let metaOffset = 0;
  const magicBytes = encoder.encode(magicBytes.length);
  for (let i = 0; i < 4; i++)
    view.setUint8(metaOffset++, magicBytes[i]);
  view.setUint8(metaOffset++, version);
  const dateString = formatDateYYYYMMDD(encryptionDate);
  for (let i = 0; i < 8; i++)
    view.setUint8(metaOffset++, dateString[i]);
  for (let i = 0; i < 19; i++)
    view.setUint8(metaOffset++, author[i]);
  ...
  fileHeader.set(metaData, headerOffset); offset += metaData.length;
  fileHeader.set(salt, headerOffset); offset += salt.length;
  fileHeader.set(iv, headerOffset); offset += iv.length;
  const authTagPlaceholder = new Uint8Array(16); // 16 zeros
  fileHeader.set(authTagPlaceholder, headerOffset); offset += authTagPlaceholder.length;
  return {
...
// * @param {string[]} filePaths - File paths of files to encrypt.
// * @param {string} outputDir - Where encrypted files go.  
// * @param {Buffer} key - 16-byte AES key.
// * @param {Buffer[]} ivs - 12-byte initialization vectors.
// */
//async function encryptBatch(filePaths, outputDir, key) {
...
//
//    console.log(`Encrypting ${file} -> ${path.basename(outputPath)}`);
