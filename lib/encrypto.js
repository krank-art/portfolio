 ...
 * ```
*/
async function encryptSets({ sets, inputDir, outputDir, silent = true }) {
  ...
  for (const set of sets) {
    ...
    for (const post of posts) {
      ...
      for (const relativePath of post) {
        ...
        if (!(await access(outputPath, constants.W_OK)))
          throw new Exception(`Cannot write encrypted file '${filepath}'`);
