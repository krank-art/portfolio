...
import { createFileHeader, deriveAES128Key } from "./encrypto-common.js";
...
export async function encryptSets({ sets, inputDir, outputDir, silent = true }) {
  ...
  for (const set of sets) {
    ...
    for (const post of posts) {
      ...
      const cryptoKey = await deriveAES128Key({ cryptoLib: crypto.webcrypto.subtle, password, salt });
      const rawKey = await crypto.webcrypto.subtle.exportKey("raw", cryptoKey);
...
async function generateKeyAES128(password, salt = crypto.randomBytes(16)) {
  // We will use a common function that utilizes the Web Crypto API, this function uses the interal NodeJS crypto module
