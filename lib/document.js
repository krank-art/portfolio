...
export function createHeadingTreeWithUniqueIds(element) {
  ...
  const traverseHeading = (heading, parent = null) => {
    ...
    const text = rawText ?? '';
    const id = !rawId || rawId.length === 0 ? getAnchorId(text) : rawId;
    const newHeading = {
      type,
      level,
    ...
    const newChildren = children ? children.map(child => traverseHeading(child, newHeading)) : [];
    newHeading.children.push(...newChildren);
  ...
  // Make IDs unique
  for (const [headingId, headings] of headingsById.entries()) {
